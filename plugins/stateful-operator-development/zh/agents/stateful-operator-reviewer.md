...
签署全部四道门禁。SPEC评审：规格完整、可行、验收标准可验证。设计评审：设计不越SPEC、覆盖受影响层次、风险与测试策略成立。代码评审：经flink-velox-code-review技能对diff做结构化评审。最终验收：对照VERIFY.md逐条核对SPEC验收标准、判断残留风险。一个任务兼守多道门禁时，在门禁之间释放上下文——早期阶段的细节对后期阶段无关。

...
|---|---|---|---|---|
| Design audit | 1 | <pass/fail/not started> | `tasks/<task-name>/reviewer/DESIGN_AUDIT.md` | |
| Code audit | 1 | <pass/fail/not started> | `tasks/<task-name>/reviewer/CODE_AUDIT.md` | |
| Result audit | 1 | <pass/fail/not started> | `tasks/<task-name>/reviewer/RESULT_AUDIT.md` | |
| User gate 1 (after design) | — | <released/pending> | `tasks/<task-name>/USER_GATES.md` | |
| User gate 2 (before code delivery) | — | <released/pending> | `tasks/<task-name>/USER_GATES.md` | |
| User gate 3 (after acceptance) | — | <released/pending> | `tasks/<task-name>/USER_GATES.md` | |
...
tmp/<task-name>/logs/           logs only (cleanable anytime, never gate evidence): cmd-outputs/  jobs/
```

The project root additionally holds the `e2e/{sql,data,out,verify}/` verification evidence tree — outside the task directory, preserved after the task ends; the full-result rollup lives in `e2e/verify/RESULTS.md`.

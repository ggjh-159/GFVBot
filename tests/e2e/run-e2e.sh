...
a "env: nine build tools probed" bash -c "jq -e '(.\"build_tools\"|keys|length)==9 and (.[\"build_tools\"]|to_entries|all(.value==true or .value==false))' '$TC/.gfvbot/env.json'"
a "env: thirteen cpp deps probed" bash -c "jq -e '(.\"cpp_deps\"|keys|length)==13 and (.[\"cpp_deps\"]|to_entries|all(.value==true or .value==false)) and .\"cpp_deps\".libevent and .\"cpp_deps\".zlib' '$TC/.gfvbot/env.json'"

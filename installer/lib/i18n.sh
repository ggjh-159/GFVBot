...
MSG_ZH[err_needs_value]='%s 需要一个值'
...
MSG_ZH[err_name_mismatch]='清单 name 与目录名不一致: %s'
MSG_EN[err_workflow_field]='%s: workflow field must be workflow.md'
MSG_ZH[err_workflow_field]='%s: workflow 字段必须为 workflow.md'
MSG_EN[err_workflow_missing]='%s: workflow.md not found'
MSG_ZH[err_workflow_missing]='%s: 缺少 workflow.md'
MSG_EN[err_unit_missing]='manifest of %s declares missing %s: %s'
MSG_ZH[err_unit_missing]='%s 的清单声明了不存在的 %s: %s'
MSG_EN[msg_skip]='%s: already installed, source unchanged — skip'
MSG_ZH[msg_skip]='%s: 已安装且源未变化，跳过'
MSG_EN[step_install]='install plugin: %s [%s] tool=%s target=%s'
MSG_ZH[step_install]='安装插件: %s [%s] AI Agent=%s 目标=%s'
...
MSG_ZH[msg_health_ok]='%s: 健康检查通过（落盘 %s 项）'
MSG_EN[step_done]='done: %s plugin(s) -> %s @ %s'
MSG_ZH[step_done]='完成: %s 个插件 -> %s @ %s'
...
MSG_ZH[err_no_records]='%s 下没有 GFVBot 安装记录'
MSG_EN[warn_no_record]='%s: no install record under %s, skipping'
MSG_ZH[warn_no_record]='%s: 在 %s 下没有安装记录，跳过'
...
MSG_EN[env_nexmark_done]='nexmark jar deployed: %s'
MSG_ZH[env_nexmark_done]='nexmark jar已部署：%s'
...
MSG_ZH[step_reclaim_tool]='领地回收: %s 下最后一个插件已移除'
MSG_EN[msg_tool_reclaimed]='territory of %s reclaimed'
MSG_ZH[msg_tool_reclaimed]='%s 的领地已回收'
...
MSG_ZH[err_unsupported_shell]='不支持的 shell: %s（支持 bash 或 zsh）'
...
MSG_ZH[lang_src_env]='环境变量 GFVBOT_LANG'
...
MSG_ZH[lang_src_locale]='系统 locale %s'
...
MSG_ZH[err_bad_lang]='只支持 en 或 zh: %s'

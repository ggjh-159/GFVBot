...
```bash
bash bin/compile.sh > /tmp/build-velox.log 2>&1
tail -5 /tmp/build-velox.log
```

Run `bin/compile.sh` relative to this skill's directory (or invoke it by absolute path from anywhere under the workspace). Check the exit code and the log tail; on failure, grep the log for the first error instead of re-running blindly.
...
- Always redirect build output to a log file and inspect it with tail/grep. A full compile streams tens of thousands of lines; pulling that into context wastes the window.

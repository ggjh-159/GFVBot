...
返回当前时刻的TIMESTAMP（无时区），即本地钟面时间；每查询求值一次，同一查询内所有行取同一值；不带括号。面向展示的本地时间。

## 用法

签名：`LOCALTIMESTAMP`——无括号的时间常量写法；类型TIMESTAMP。

无参数。

返回：TIMESTAMP；查询开始时的本地钟面时间，同一查询内固定不变。
...
| 函数定义与类型推导 | `BuiltInFunctionDefinitions`的`LOCAL_TIMESTAMP`条目（SCALAR） |

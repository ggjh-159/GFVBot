...
返回第一个非NULL参数，全部参数为NULL才得NULL；所有参数须能统一为同一类型。可选列的兜底链。
...
| 参数 | 类型 | 说明 |
|---|---|---|
| v1, v2, ... | 可统一类型 | 两个及以上参数，自左向右考察 |

返回：各参数的公共类型；第一个非NULL参数，全部为NULL才得NULL。
...
| 函数定义与类型推导 | `BuiltInFunctionDefinitions`的`COALESCE`条目（SCALAR） |
| 求值逻辑 | flink-table-runtime的scalar/`CoalesceFunction`的eval()（经`BridgingSqlFunctionCallGen`调用） |

## velox实现

velox表达式内核将其作为特型`coalesce`处理（`velox/expression/RegisterSpecialForm.cpp`），无独立函数注册。

...
Two-argument COALESCE: `IFNULL(a, b)` yields a when a is not NULL, else b. The concise defaulting form for a single nullable column.

## Usage

Input: `IFNULL(a, b)` — a, b of a common type.
...
The route of `IFNULL` from SQL text to the executing operator (Flink 1.19.2):
...
2. **Definition** — the BuiltInFunctionDefinitions entry at BuiltInFunctionDefinitions.java:132, registered under the name "IFNULL", kind SCALAR; the planner binds the parsed call to this definition.
3. **Planning** — No dedicated rewrite; as a plain RexCall it moves with the generic rules — filter/project push-down, CalcMergeRule, constant folding (ExpressionReducer) when fully literal.
4. **Codegen** — BridgingSqlFunctionCallGen invokes eval() on the table-runtime class scalar/IfNullFunction (flink-table-runtime, new-stack carrier).

...
按指定长度取SHA-2摘要——hashLength为224、256、384或512（十六进制字符数相同）；0选256；其他长度会被拒绝。一个拼写覆盖整个SHA-2系列。

## 用法

输入：`SHA2(s, hashLength)`——s为STRING，hashLength为INT。
...
`SHA2`从SQL文本到执行算子的路径（Flink 1.19.2）：

1. **解析**——函数形式，解析器产出SqlNode，算子锚点为`FlinkSqlOperatorTable.SHA2`（FlinkSqlOperatorTable.java:577）。
2. **定义**——BuiltInFunctionDefinitions.java:2100处的注册条目，注册名`"sha2"`，kind为SCALAR；planner把解析出的调用绑定到该定义。

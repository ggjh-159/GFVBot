...
当前时刻，类型为TIMESTAMP WITH LOCAL TIME ZONE，按会话时区呈现；每查询求值一次；不带括号。与NOW()同义。

## 用法

输入：`CURRENT_TIMESTAMP`——不带括号；TIMESTAMP_LTZ。
...
`CURRENT_TIMESTAMP`从SQL文本到执行算子的路径（Flink 1.19.2）：
...
2. **定义**——BuiltInFunctionDefinitions.java:1772处的注册条目，注册名`"currentTimestamp"`，kind为SCALAR；planner把解析出的调用绑定到该定义。

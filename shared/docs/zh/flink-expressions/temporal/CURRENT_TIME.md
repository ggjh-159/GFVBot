...
当前的当天时刻，每查询求值一次，不带括号。查询时的时段路由。

## 用法

输入：`CURRENT_TIME`——不带括号。
...
`CURRENT_TIME`从SQL文本到执行算子的路径（Flink 1.19.2）：
...
2. **定义**——BuiltInFunctionDefinitions.java:1758处的注册条目，注册名`"currentTime"`，kind为SCALAR；planner把解析出的调用绑定到该定义。

...
当前本地当天时间（无时区），每查询求值一次，不带括号。面向展示的本地时间。
...
`LOCALTIME`从SQL文本到执行算子的路径（Flink 1.19.2）：
...
2. **定义**——BuiltInFunctionDefinitions.java:1765处的注册条目，注册名`"localTime"`，kind为SCALAR；planner把解析出的调用绑定到该定义。

...
`COSH`从SQL文本到执行算子的路径（Flink 1.19.2）：

1. **解析**——函数形式，解析器产出SqlNode，算子锚点为`FlinkSqlOperatorTable.COSH`（FlinkSqlOperatorTable.java:362）。
2. **定义**——BuiltInFunctionDefinitions.java:1600处的注册条目，注册名`"cosh"`，kind为SCALAR；planner把解析出的调用绑定到该定义。

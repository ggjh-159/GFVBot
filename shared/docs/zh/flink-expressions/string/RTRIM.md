...
`RTRIM`从SQL文本到执行算子的路径（Flink 1.19.2）：

1. **解析**——函数形式，解析器产出SqlNode，算子锚点为`FlinkSqlOperatorTable.RTRIM`（FlinkSqlOperatorTable.java:924）。
2. **定义**——BuiltInFunctionDefinitions.java:1132处的注册条目，注册名`"rtrim"`，kind为SCALAR；planner把解析出的调用绑定到该定义。

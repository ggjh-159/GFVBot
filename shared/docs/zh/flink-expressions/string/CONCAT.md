...
把参数从左到右拼接；任一参数为NULL则返回NULL。拼装展示字符串与复合键。

## 用法

输入：`CONCAT(s1, s2, ...)`——两个及以上字符串参数。
...
`CONCAT`从SQL文本到执行算子的路径（Flink 1.19.2）：

1. **解析**——函数形式，解析器产出SqlNode，算子锚点为`FlinkSqlOperatorTable.CONCAT_FUNCTION`（FlinkSqlOperatorTable.java:226）。
2. **定义**——BuiltInFunctionDefinitions.java:924处的注册条目，注册名`"concat"`，kind为SCALAR；planner把解析出的调用绑定到该定义。

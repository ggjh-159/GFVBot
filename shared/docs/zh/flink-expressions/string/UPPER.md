...
把字符串转为大写。来源大小写混杂时，在比较或分组前做大小写归一。

## 用法

输入：`UPPER(s)`——s为STRING/CHAR；NULL得NULL。
...
`UPPER`从SQL文本到执行算子的路径（Flink 1.19.2）：

1. **解析**——函数形式，解析器产出SqlNode，算子锚点为`FlinkSqlOperatorTable.UPPER`（FlinkSqlOperatorTable.java:1181）。
2. **定义**——BuiltInFunctionDefinitions.java:870处的注册条目，注册名`"upper"`，kind为SCALAR；planner把解析出的调用绑定到该定义。

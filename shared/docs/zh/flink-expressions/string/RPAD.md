...
在s右侧用pad补齐到恰好len个字符；超长输入会被截到len。

## 用法

输入：`RPAD(s, len, pad)`——len为INT。
...
`RPAD`从SQL文本到执行算子的路径（Flink 1.19.2）：

1. **解析**——函数形式，解析器产出SqlNode，算子锚点为`FlinkSqlOperatorTable.RPAD`（FlinkSqlOperatorTable.java:403）。
2. **定义**——BuiltInFunctionDefinitions.java:962处的注册条目，注册名`"rpad"`，kind为SCALAR；planner把解析出的调用绑定到该定义。

...
按指定字符集把字节解码回STRING。ENCODE的逆操作。

## 用法

输入：`DECODE(bytes, charset)`——bytes为VARBINARY；返回STRING。
...
`DECODE`从SQL文本到执行算子的路径（Flink 1.19.2）：

1. **解析**——函数形式，解析器产出SqlNode，算子锚点为`FlinkSqlOperatorTable.DECODE`（FlinkSqlOperatorTable.java:882）。
2. **定义**——BuiltInFunctionDefinitions.java:1023处的注册条目，注册名`"decode"`，kind为SCALAR；planner把解析出的调用绑定到该定义。
